...
function selectStatus(val) {
  ...
  document.querySelector(`.sp-${val}`)?.classList.add('active');
}
...
function submitReport(e) {
  ...
  DB.recentReports.unshift({
    station, product: product.replace(/([A-Z])/g, ' $1').trim(),
    status: selectedStatus, queue, time: 'Just now',
    verified: false, user: 'You'
  });
